...
%
    ...
    sql( 'CREATE TABLE user(user_id INTEGER PRIMARY KEY, name TEXT)' );
    sql( 'CREATE TABLE data(user_id INTEGER, activity TEXT, FOREIGN KEY (user_id) REFERENCES user(user_id))' );
    ...
    [~, ~, ~, row_id] = sql( 'INSERT INTO user (name) VALUES (:name)', ...
    ...
    sql( 'INSERT INTO data (user_id,activity) VALUES (:user_id,:activity)', dat );
    ...
    sql( 'result_type', 0 ); % structs (default)
    test_type_0 = sql( 'SELECT name, activity, user_id FROM data JOIN user USING(user_id)' );
    
    sql( 'result_type', 2 ); % cell matrix
    test_type_2 = sql( 'SELECT name, activity, user_id FROM data JOIN user USING(user_id)' );
    ...
    lhs = sql( ['SELECT name, activity FROM data JOIN user USING(user_id) ', ...
    ...
    rhs = sql( ['SELECT name, activity FROM data JOIN user USING(user_id) ', ...
    ...
    sql( 'DELETE FROM data WHERE user_id=:myID', struct('myID', row_id(1) ) )
    % now you can delete also the corresponding user
    sql( 'DELETE FROM user WHERE user_id=:myID', struct('myID', row_id(1) ) )
